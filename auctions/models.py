...
class Listing(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="createdBy")
    status = models.CharField(max_length=6)

    def __str__(self):
        return f"title:{ self.title } imageURL:{ self.imageURL } description:{ self.description } startingBid:{ self.startingBid } category:{ self.category } createdBy: { self.user }"
...
class CommentForm(forms.Form):
    ...
    comment = forms.CharField(
        widget=forms.Textarea(attrs={'class': 'form-control'}), max_length=450, label="Comment")
...
class Comment(models.Model):
    ...
    commentBy = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="commentBy")

    def __str__(self):
        return f"User: { self.commentBy } title: { self.title } comment: { self.comment }"
